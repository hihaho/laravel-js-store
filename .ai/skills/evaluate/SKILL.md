...
**Note:** This skill is a completion-level activity. It runs the full `backend-quality` skill (including Rector, PHPStan, and the full test suite). Do not use this skill mid-feature — only when the implementation is done.
...
- Rector: `vendor/bin/rector process` ran with 0 files changed
...
- Tests: `vendor/bin/phpunit` ran with 0 failures (full suite or all relevant tests)
...
| **Cross-version compat** | Works on PHP 8.2-8.4 and Laravel 12-13 |
